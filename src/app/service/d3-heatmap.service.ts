...
export class D3HeatmapService {
    ...
    public initialize(domElement: string, config: HeatmapConfig, sectorProgressFn: (sector: Sector) => number): void {
        this.config = config;
        ...
        const bbWidth = imageWidth - Math.max(margin.left + margin.right, margin.top + margin.bottom) * 2;
        ...
        this.svg = d3.select(domElement)
            ...
            .append('g')
            .attr('transform', `translate(${margin.left + this.segmentLabelHeight}, ${margin.top + this.segmentLabelHeight})`) as any;
    }
    ...
    public render(
        ...
        const arc = d3.arc<SVGPathElement, Sector>()
            .innerRadius((d, i) => this.innerRadius + Math.floor(i / numSegments) * this.segmentHeight)
            .outerRadius((d, i) => this.innerRadius + this.segmentHeight + Math.floor(i / numSegments) * this.segmentHeight)
        ...
        const cursors = this.svg.append('g')
    ...
    public recolorSector(index: number, progressValue: number): void {
        ...
        this.svg.select('#index-' + index).attr(
            'fill',
